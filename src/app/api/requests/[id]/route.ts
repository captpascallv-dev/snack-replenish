...
export async function PATCH(
  ...
  if (body.action === "fulfill") {
    ...
    await createFulfillment(id, { orderDate, supplier, kg, caseSpec, caseCount, orderNotes });
    ...
    await createReceipt(id, { arrivalDate, actualQuantity: Number(actualQuantity), feedback });
  } else {
    return NextResponse.json({ error: "未知操作，action 须为 fulfill 或 receive" }, { status: 400 });
  }

  return NextResponse.json(await getRequestById(id));
}
